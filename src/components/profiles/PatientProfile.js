...
export default class PatientProfile extends Component {
  ...
  componentDidMount() {
    ...
    if (uid !== null) {
      ...
      axios
        .get(URLs.blog_api + "patient/gethistory?user_id=" + uid)
        .then((response) => {
          if (response.status === 200) {
            this.setState({
              userHistories: response.data.history,
            });
          }
        })
        .catch((error) => {
        ...
        <div class="tab-content" id="myTabContent">
          ...
          >
            ...
            <table class="table table-hover">
              ...
              <tbody>
                {this.state.userHistories.map((video) => (
                  <tr>
                    <th>{video.date.substring(0, 10)}</th>
                    <td>{video.doctor_name}</td>
                    <td>{video.diseases.length}</td>
                    <td>{video.tests.length}</td>
                    <td>{video.medicines.length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
