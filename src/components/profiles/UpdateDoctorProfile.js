...
export default class UpdateDoctorProfile extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    axios
      .get(URLs.blog_api + "doctor/categories")
      .then((response) => {
    ...
    axios
      .get(URLs.blog_api + "doctor/getspecific?id=" + uid)
      .then((response) => {
        if (response.status === 200) {
          this.setState({
            submitUrl: "doctor/edit",
            about: response.data.profile.about,
            bmdc_code: response.data.profile.bmdc_code,
  ...
  handleSubmit = (e) => {
    ...
    if (this.state.bmdc_code === "") {
      this.setState({
        error: "BMDC Code can't be empty",
        submitLoading: false,
      });
      return;
    }
    ...
    axios
      .post(
        ...
        querystring.stringify({
          user_id: uid,
          bmdc_code: self.state.bmdc_code,
    ...
    return (
      <div className="overflow-hidden">
        <div className="container my-4 pb-4">
          ...
          {this.state.submitUrl === "doctor/add" ? (
            <div class="form-group px-2">
              ...
              <input
                ...
                onChange={(e) => {
                  this.setState({ bmdc_code: e.target.value });
                }}
              />
            </div>
          ) : (
